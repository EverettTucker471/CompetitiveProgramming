...
int main() {
    ...
    while (cin >> n) {
        ...
        // Generate a suffix array for pat
        // For each string in s, binary search for all occurences
        vector<int> sa = construct_suffix_array(pat);
        ...
        for (string sub : s) {
            ...
            while (left < right) {
                ...
                if (sub <= pat.substr(sa[mid], sub.size())) {
                    right = mid - 1;
                } else {
                    left = mid;
                }
            ...
            if (sub == pat.substr(sa[lower_bound], sub.size())) lower_bound--;
            if (sub == pat.substr(sa[upper_bound], sub.size())) upper_bound++;

            vector<int> occur;
            for (int i = lower_bound + 1; i < upper_bound; i++) {
                occur.push_back(sa[i]);
            }
